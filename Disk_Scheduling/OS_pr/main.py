...
def animate_single(sequence, label):
    fig, ax = plt.subplots(figsize=(10, 5))
    line, = ax.plot([], [], marker='o', linestyle='-', color='#4CAF50', label=label)
    ax.set_xlim(0, len(sequence) - 1)
    ax.set_ylim(min(sequence) - 10, max(sequence) + 10)
    ax.set_title(f"{label} Disk Scheduling", fontsize=14)
    ax.set_xlabel("Step", fontsize=12)
    ax.set_ylabel("Cylinder", fontsize=12)
    ax.grid(True)
    ax.legend()
    ...
    ani = animation.FuncAnimation(fig, update, frames=len(sequence), interval=500, repeat=False)
    plt.tight_layout()
    plt.show()
    ...
    def run_visualization(self):
        try:
            ...
            if self.compare_var.get():
                ...
                animate_single(sequence, algorithm)
